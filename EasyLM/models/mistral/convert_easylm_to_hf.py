...
# This script converts Mistral model checkpoint trained by EsayLM to the
# HuggingFace transformers Mistral PyTorch format, which can then be loaded
...
def write_model(loaded, model_path, model_size):
    ...
    for layer_i in tqdm(range(n_layers), desc='converting weights'):
        ...
        state_dict = {
            ...
            f"model.layers.{layer_i}.self_attn.k_proj.weight": permute(
                loaded[f"transformer.h.{layer_i}.attention.wk.kernel"], is_q=True
            ),
    ...
    model = MistralForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.float16)
