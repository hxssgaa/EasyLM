...
from EasyLM.models.mistral.mistral_model import MistralConfig, FlaxMistralForCausalLM
...
def main(argv):
    ...
    prefix_tokenizer = MistralConfig.get_tokenizer(
        FLAGS.tokenizer, truncation_side='left', padding_side='left'
    )
    tokenizer = MistralConfig.get_tokenizer(
    ...
    with jax.default_device(jax.devices("cpu")[0]):
        ...
        hf_model = FlaxMistralForCausalLM(
            mistral_config,
    ...
    model_ps = match_partition_rules(
        MistralConfig.get_partition_rules(), params
    )
    ...
    def forward_loglikelihood(params, rng, batch):
        ...
        logits = hf_model.module.apply(
            ...
            deterministic=True, rngs=rng_generator(mistral_config.rng_keys()),
        ).logits
        # if mistral_config.n_real_tokens is not None:
        #   logits = logits.at[:, :, mistral_config.n_real_tokens:].set(-1e8)
